...
public class C2 extends JFrame {
	...
	public C2() {
		...
		btnNewButton.addActionListener(new ActionListener() {
			...
			if((rn.length()==10)&&(p.length()<11 && p.length()>0)&&((h.length()<11) &&(h.length()>0))&&(b!=""))
			{
			Socket s=null;
			try{
			...
			else { JOptionPane.showMessageDialog(null,"Not register");
			}
		...
		/*	if((rn.length()==10)&&(p.length()<11 && p.length()>0)&&((h.length()<11) &&(h.length()>0))&&(b!=""))
			...
			Class.forName("com.mysql.jdbc.Driver");	
			 ...
			 st.executeUpdate("insert into cse(Regd_no,Password,Hint) values('"+rn+"','"+p+"','"+h+"')");
			 JOptionPane.showMessageDialog(null,"Sucessfully register");
