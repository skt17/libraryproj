	...
	public Part2() {
		...
		btnNewButton.addActionListener(new ActionListener() {
			...
			n[7]=textField_67.getText();
				...
				while(true){
					try{
						if(b[i].equals(c)){break;}
						Class.forName("com.mysql.jdbc.Driver");	
						 Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/book","root","");
					Statement st=con.createStatement();
					if(s.equals("ele")){
					st.executeUpdate("insert into ele (Book_name,Author,Pub_name,Total_book,Avail_book) values('"+b[i]+"','"+a[i]+"','"+p[i]+"','"+Integer.valueOf(n[i])+"','"+Integer.valueOf(n[i])+"')");
					}
					else if(s.equals("mec")){
						st.executeUpdate("insert into mec (Book_name,Author,Pub_name,Total_book,Avail_book) values('"+b[i]+"','"+a[i]+"','"+p[i]+"','"+Integer.valueOf(n[i])+"','"+Integer.valueOf(n[i])+"')");
						}
					else if(s.equals("civ")){
						st.executeUpdate("insert into civ (Book_name,Author,Pub_name,Total_book,Avail_book) values('"+b[i]+"','"+a[i]+"','"+p[i]+"','"+Integer.valueOf(n[i])+"','"+Integer.valueOf(n[i])+"')");
						}
					else if(s.equals("cse")){
						st.executeUpdate("insert into cse (Book_name,Author,Pub_name,Total_book,Avail_book) values('"+b[i]+"','"+a[i]+"','"+p[i]+"','"+Integer.valueOf(n[i])+"','"+Integer.valueOf(n[i])+"')");
						}
					else if(s.equals("common")){
						st.executeUpdate("insert into common (Book_name,Author,Pub_name,Total_book,Avail_book) values('"+b[i]+"','"+a[i]+"','"+p[i]+"','"+Integer.valueOf(n[i])+"','"+Integer.valueOf(n[i])+"')");
						}
				
				i=i+1;
					}
					catch(Exception e){z=1;System.out.print(e);break;}
